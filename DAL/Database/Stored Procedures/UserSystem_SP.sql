...
--modify user
--drop procedure SP_MODIFYUSERPASSWORDSYSTEM
CREATE PROCEDURE SP_MODIFYUSERPASSWORDSYSTEM
	@USERSYSTEM_ID NUMERIC,
...
BEGIN
       SELECT US.USERSYSTEM_ID,US.ID,US.NAME,US.SURNAME,US.PROGRAM_ID,P.NAME AS PROGRAM,US.PHONE
      ,US.CELLPHONE,US.MAIL,US.ROLE_ID,R.DESCRIPTION,US.STATE, US.Set_Password, US.PASSWORD
       FROM USERSYSTEM US, PROGRAM P,ROLE R
	   WHERE US.DELETED=1 AND US.STATE=1 AND US.PROGRAM_ID=P.PROGRAM_ID AND US.ROLE_ID=R.ROLE_ID AND MAIL=@MAIL AND PASSWORD=@PASSWORD;
END;
GO
...
--drop procedure RESETPASSWORDSECURITY
CREATE PROCEDURE RESETPASSWORDSECURITY
	@USERSYSTEM_ID NUMERIC,
	@PASSWORD VARCHAR(100)
AS 
BEGIN
