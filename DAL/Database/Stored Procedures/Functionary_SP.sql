...
BEGIN
	SELECT F.FUNCTIONARY_ID, ID, F.NAME,SURNAME,PHONE,CELLPHONE,MAIL,F.STATE,F.PROGRAM_ID, P.NAME
	FROM FUNCTIONARY F, PROGRAM P
	WHERE F.DELETED = 1 and F.PROGRAM_ID=P.PROGRAM_ID and F.PROGRAM_ID = @PROGRAM_ID;
END;
GO
...
CREATE PROCEDURE GET_FUNCTIONARY_INFO
@ID NUMERIC
AS
BEGIN
	SELECT ID, NAME, SURNAME, PHONE, CELLPHONE, MAIL, PROGRAM_ID 
	FROM FUNCTIONARY
	WHERE ID = @ID AND
	DELETED = 1;
END;
