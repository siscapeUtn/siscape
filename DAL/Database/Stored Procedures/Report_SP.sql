...
END;
GO
--drop proc SP_REPORT_ACADEMIC_OFFER
...
--drop proc SP_REPORT_WAITING_LIST
CREATE PROCEDURE SP_REPORT_WAITING_LIST
@PERIOD_ID NUMERIC,
@IS_CONTACT NUMERIC
AS
BEGIN
	SELECT W.NAME, W.SURNAME , W.PHONE, W.CELLPHONE, W.MAIL, C.DESCRIPTION, (S.DESCRIPTION+' '+S.TYPESHEDULE) AS DAY
	FROM WAITING_LIST W, ACADEMIC_OFFER AO, COURSE C, PERIOD P, SCHEDULE S
	WHERE AO.PERIOD_ID = @PERIOD_ID AND 
	C.COURSE_ID = AO.COURSE_ID AND 
	W.ACADEMIC_OFFER_ID = AO.ACADEMIC_OFFER_ID AND 
	AO.PERIOD_ID = P.PERIOD_ID AND
	AO.SCHEDULE_ID = S.SCHEDULE_ID AND
